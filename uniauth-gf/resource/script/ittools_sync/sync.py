...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('sync.log', encoding='utf-8')  # 同时输出到文件
        ]
    )
    # 确保输出立即显示
...
def validate_pg_env() -> None:
    ...
    for name, value in [
        ...
        if not value:
            missing.append(name)
    if missing:
        logger.error(f"错误：缺少必需的PostgreSQL环境变量：{', '.join(missing)}")
        raise SystemExit(1)
...
try:
    ...
    logger.error(f"错误：环境变量 QUERY_API_KEYS 格式无效: {e}")
    raise SystemExit(1)
...
async def fetch_ad_users() -> list[dict[str, Any]]:
    ...
    async def fetch_one(session: ClientSession, api_key: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error(f"错误：ITTools API请求失败 (key: ...{api_key[-4:]}): {e}")
            return None
    ...
    logger.info("（2/3）AD数据获取成功，正在解析数据...")
...
async def sync_to_postgres(users: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        async with conn.transaction():
            logger.info("(3/3) 正在写入数据到PostgreSQL...")
            ...
            for user in users:
                ...
                await conn.execute(
                    """
                      INSERT INTO userinfos_user_infos (
                          upn, email, display_name, sam_account_name, school_status, identity_type, employee_id,
        ...
        logger.info("数据写入PostgreSQL成功。")

        logger.info("清理PostgreSQL过期数据...")
        result = await conn.execute(
            "DELETE FROM userinfos_user_infos WHERE updated_at < NOW() - INTERVAL '7 days' RETURNING upn"
        )
        logger.info(f"已从PostgreSQL清理 {len(result)} 条过期记录。{result}")
    except Exception as e:
        logger.error(f"同步到PostgreSQL时出错: {e}")
    finally:
        if conn:
            await conn.close()
