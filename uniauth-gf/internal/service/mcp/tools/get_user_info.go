...
type UserinfosUserInfos struct {
	...
	CreatedAt                  string   `json:"createdAt"                  orm:"created_at"                     description:"创建时间 - 记录创建时间。"`                                                                                                      // 创建时间 - 记录创建时间。
	UpdatedAt                  string   `json:"updatedAt"                  orm:"updated_at"                     description:"更新时间 - 记录最后更新时间。"`                                                                                                    // 更新时间 - 记录最后更新时间。
}
...
func RegisterGetUserInfo(s *server.MCPServer) error {
	tool := mcp.NewTool("get_user_info",
		...
		mcp.WithOutputSchema[UserinfosUserInfos](),
	)

	// Add the calculator handler
	handler := func(ctx context.Context, request mcp.CallToolRequest, args v1.GetOneReq) (UserinfosUserInfos, error) {
		// Using helper functions for type-safe argument access
		res, err := userinfos.NewV1().GetOne(ctx, &args)
		if err != nil {
			return UserinfosUserInfos{}, gerror.Wrap(err, "获取用户信息失败")
		}
		if res == nil {
