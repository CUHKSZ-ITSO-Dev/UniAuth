...
type ChatPreCheckOneStopReq struct {
    g.Meta    `path:"/internal/chat/oneStop" tags:"Auth/Chat" method:"post" summary:"对话服务一站式权限预检查" dc:"对话服务开启计费流程前的一站式权限检查，会进行以下检查：<br>1. 检查配额池是否存在；<br>2. 检查配额池是否被禁用；<br>3. 检查用户有没有权限使用这个配额池；<br>4. 检查配额池有没有权限使用这个 Svc 和 Product。"`
...
type GetAvailableModelForQuotaPoolReq struct {
    g.Meta    `path:"/internal/chat/quotaPools/models" tags:"Auth" method:"get" summary:"获取所属配额池的可用模型" dc:"动态获取指定配额池的可用模型。"`
