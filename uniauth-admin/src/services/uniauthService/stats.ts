...
/** 对话服务产品使用次数统计接口（详细） 可以传入最近N天参数。 GET /billing/admin/stats/chat/usage/chart */
export async function getBillingStatsChatUsageChart(
  ...
  return request<API.NDaysProductUsageChartRes>(
    "/billing/admin/stats/chat/usage/chart",
    {
      method: "GET",
      params: {
...
/** 对话服务产品使用次数统计接口（聚合） 可以传入最近N天参数。 GET /billing/admin/stats/chat/usage/group */
export async function getBillingStatsChatUsageGroup(
  ...
  return request<API.NDaysProductUsageGroupRes>(
    "/billing/admin/stats/chat/usage/group",
    {
      method: "GET",
      params: {
        // N has a default value: 7
